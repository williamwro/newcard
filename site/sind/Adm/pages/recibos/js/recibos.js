...
$('#btnExibir').click(function () {
    ...
    // PARA APARECER O TOTAL DOS RECEBOS TEM QUE DESCOMENTAR A LINHA ABAIXO
    // TOTAL SENDO SOMADO SEM OS CONVENIOS COM PROLABORE ZERO
    //$('#panel_total').show();
    $.getJSON( "../Adm/pages/recibos/total_sem_pro.php", {mes: $("#C_mes").val()}, function( data ) {
        var valor;
        $.each(data, function (index, value) {
            valor = parseFloat(data[1].total);
            valor = valor.toLocaleString("pt-BR", { minimumFractionDigits: 2 });
            $('#total_recibos').val(valor);
        });
    });
});
